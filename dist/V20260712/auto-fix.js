(function () {
    ...
    const SCRIPT_NAME = 'Script dọn nhãn chính văn tầng';
    const LOADED_FLAG = '__floor_body_tag_cleanup_script_loaded__';
    const GLOBAL_API_NAME = '__fixMessageTagBlocks__';
    // Chỉ sửa tại đây: tương thích tag <think> mới nhất, bảo đảm tách đúng vùng suy nghĩ và định vị phần trước <now_plot>.
    ...
    function normalizeTagBlocks(message) {
        ...
        // Giữ logic lõi: khi phát hiện card ngoài, bóc tag cũ ở đầu/cuối rồi để `normalized += <now_plot>` và `</now_plot>` bọc lại thống nhất bên dưới.
        ...
        if (hasNowPlotPair && !shouldRewrapNowPlot) {
            ...
            // Tuyệt đối không động vào xử lý tag cuối; giữ nguyên logic gốc.
            normalized += `</now_plot>`;
        }
    ...
    function unload() {
        while (listenerStops.length > 0) {
            ...
            try {
                ...
                console.warn(`[${SCRIPT_NAME}] Gỡ listener thất bại`, error);
            }
        }
    ...
    function resetMount() {
        ...
        console.log(`[${SCRIPT_NAME}] Đã reset mount`);
    }

    const init = async () => {
        const root = getRootWindow();
        if (root[LOADED_FLAG]) {
            console.log(`[${SCRIPT_NAME}] Phát hiện instance cũ, gỡ trước rồi tải lại`);
            try {
                ...
                console.warn(`[${SCRIPT_NAME}] Gỡ instance cũ thất bại, tiếp tục thử tải lại`, error);
                root[LOADED_FLAG] = false;
            }
        }
